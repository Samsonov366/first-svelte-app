...
<svelte:head>
  <title>{$source ? `${$title} - ${$artist}` : "Piano"}</title>
</svelte:head>
...
<div class="container">
  <div class="card">
    ...
    <div class="card__actions">
      <button type="button" on:click={prevSong}
        ><i class="fas fa-fw fa-backward" /></button
      >
      <button type="button" on:click={playAudio} class="play"
        ><i
          class="fas fa-fw fa-{!$isPlay || ended ? 'play' : 'pause'}"
        /></button
      >
      <button type="button" on:click={nextSong}
        ><i class="fas fa-fw fa-forward" /></button
      >
    </div>
  ...
  <div class="card-playlist" style="height: 556px;">
    ...
    <div
      ...
      style="padding-bottom: {songs.length > 5 ? '1em' : 0}; "
    >
